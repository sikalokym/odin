...
def fetch_all_cpam_data(country_code, year=None, start_model_year=''):
    if year:
        ...
        if not all_years or all_years == []:
            logger.error(f'No model years found for country - {country_code}')
            return
        else:
            all_years = all_years['Years']
    sw = ''
    for year in all_years:
        cpam_processing_logger.info('Starting CPAM data fetching', extra={'country': country_code, 'year': year})
        for car in cpam.get_car_types(year, country_code)['DataRows']:
            try:
                fetch_cpam_data(year, car['Type'], country_code, sw)
            except Exception as e:
                cpam_processing_cartype_logger.error(f'Error fetching car type: {e}', extra={'country': country_code, 'year': year, 'cartype': car['Type']})
    
    logger.info(f'CPAM data fetching completed for country - {country_code}')

def process_all_cpam_data(country_code, start_model_year=0):
    logger.info('Starting CPAM data preprocessing')
    folder = f"{os.getcwd()}/dist/cpam_data_prod/{country_code}/"
    for sub_folder in os.listdir(folder):
        ...
        for subsub_folder in os.listdir(new_folder):
            ...
            try:
                ...
                logger.error(f'Error processing car type {subsub_folder}: {e}')
    
    logger.info('CPAM data preprocessing completed')

def ingest_all_cpam_data(country_code, start_model_year=0):
    logger.info('Starting CPAM data update')
    folder = f"{os.getcwd()}/dist/cpam_data_prod/{country_code}/"
    for sub_folder in os.listdir(folder):
        new_folder = folder + sub_folder + '/'
        if not os.path.isdir(new_folder) or int(sub_folder) < start_model_year:
        ...
        for subsub_folder in os.listdir(new_folder):
            ...
            try:
                for _ in range(3):
                    try:
                        cpam_processing_cartype_logger.info('Data updating car type', extra={'country': country_code, 'year': sub_folder, 'cartype': subsub_folder})
                        ingest_cpam_data(sub_folder, subsub_folder, country_code)
                        cpam_processing_cartype_logger.info('Data updating completed car type', extra={'country': country_code, 'year': sub_folder, 'cartype': subsub_folder})
                        break
                    except Exception as e:
                ...
                cpam_processing_cartype_logger.info('Visa data updating car type', extra={'country': country_code, 'year': sub_folder, 'cartype': subsub_folder})
                ingest_visa_data(country_code, df_processed)
                cpam_processing_cartype_logger.info('Visa data updating completed car type', extra={'country': country_code, 'year': sub_folder, 'cartype': subsub_folder})
    ...
    logger.info('CPAM data ingestion completed', extra={'country_code': country_code})
...
def fetch_cpam_data(year, car_type, country_code, sw):
    ...
    cpam_processing_cartype_logger.info('Fetching data car type', extra={'country': country_code, 'year': year, 'cartype': car_type})
    ...
    if market_dict_auth.empty:
        cpam_processing_cartype_logger.warn('No data found for the market', extra={'country': country_code, 'year': year, 'cartype': car_type})
        return
    ...
    if global_dependency_auth.empty:
        cpam_processing_cartype_logger.warn('No dependency data found for the global market', extra={'country': country_code, 'year': year, 'cartype': car_type})
    else:
        global_dependency_auth = global_dependency_auth.explode('FeatureCode')
    
    if market_dependency_auth.empty:
        cpam_processing_cartype_logger.warn('No dependency data found for the market', extra={'country': country_code, 'year': year, 'cartype': car_type})
    else:
    ...
    folder = f"{os.getcwd()}/dist/cpam_data_prod/{country_code}/{year}/{car_type}/raw"
    ...
    cpam_processing_cartype_logger.info('Data fetched for car type.', extra={'country': country_code, 'year': year, 'cartype': car_type})
...
def preprocess_cpam_data(year, car_type, country_code):
    ...
    cpam_processing_cartype_logger.info('Processing car type.', extra={'country': country_code, 'year': year, 'cartype': car_type})
    ###############################################################################################################################################################################################################
    ...
    ###############################################################################################################################################################################################################
    target_folder = f"{os.getcwd()}/dist/cpam_data_prod/{country_code}/{year}/{car_type}/processed"
    # Create the folder if it doesn't exist
    ...
    ###############################################################################################################################################################################################################
    cpam_processing_cartype_logger.info('Data processed for car type.', extra={'country': country_code, 'year': year, 'cartype': car_type})
...
def ingest_cpam_data(year, car_type, country_code):
    folder = f"{os.getcwd()}/dist/cpam_data_prod/{country_code}/{year}/{car_type}/processed"
    if not os.path.exists(folder):
        logger.error(f'Folder {folder} does not exist', extra={'country_code': country_code})
        return
