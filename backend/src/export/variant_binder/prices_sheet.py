...
def get_sheet(ws, df_valid_pnos, df_sales_versions, title, time, df_engines_types, country):
    ...
    for _, row in df_engines_types.iterrows():
        ...
        df_group_gb['MarketText'] = df_group_gb['Code'].apply(lambda x: df_gb[df_gb['Code'] == x].iloc[0]['MarketText'])
        
        df_group_en = pd.DataFrame()
        df_group_en['Code'] = group['Engine'].drop_duplicates()
        df_group_en['MarketText'] = df_group_en['Code'].apply(lambda x: df_en[df_en['Code'] == x].iloc[0]['MarketText'])
        curr_row = insert_engines_type_title(ws, type, curr_row)
        curr_row = insert_table(ws, group, df_group_sv, df_group_gb, df_group_en, curr_row)
        curr_row += 1
...
def insert_table(ws, df_price, df_sv, df_gb, df_en, curr_row):
    ...
    df_sv['FullCode'] = prepend_string + df_sv['Code']
...
def insert_meta_column(ws, curr_row, df_sales_version):
    ...
    alternating_list = [item for pair in zip(df_sales_version['MarketText'], df_sales_version['FullCode']) for item in pair]
...
def fetch_vb_price_data(country, df_valid_pnos, time):
    ...
    df_gb = df_gb[['Code', 'MarketText']]
    df_gb = df_gb.drop_duplicates(subset=['Code'], keep='last')
    ...
    df_en = df_en[['Code', 'MarketText']]
    df_en = df_en.drop_duplicates(subset=['Code'], keep='last')
